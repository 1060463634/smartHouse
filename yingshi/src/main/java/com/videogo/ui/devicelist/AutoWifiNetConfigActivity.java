...
//        }
    ...
    private void showSelectNetConfig() {
        ...
        // 若已查询到设备信息，则按照 （AP > SmartConfig > 声波） 的优先级自动选择配网方式
        // 若未查询到设备信息，不确定设备支持的配网能力，需要用户根据未配网状态下指示灯的闪烁方式选择配网方式
        Intent toConfigNetPageIntent = null;
        boolean notFindAvailableConfigNetWay = false;

        // ap配网
        if (support_Ap) {
            toConfigNetPageIntent = new Intent(AutoWifiNetConfigActivity.this, APWifiConfigActivity.class);
            toConfigNetPageIntent.putExtra("support_Ap", true);
        }else if (support_Wifi) {
            toConfigNetPageIntent = new Intent(AutoWifiNetConfigActivity.this, AutoWifiConnectingActivity.class);
            toConfigNetPageIntent.putExtra("support_Wifi", true);
        }else if (support_sound_wave) {
            toConfigNetPageIntent = new Intent(AutoWifiNetConfigActivity.this, AutoWifiConnectingActivity.class);
            toConfigNetPageIntent.putExtra("support_sound_wave", true);
        }else{
            notFindAvailableConfigNetWay = true;
        }

        // 跳转到对应的配网界面
        if (!notFindAvailableConfigNetWay){
            toConfigNetPageIntent.putExtra(WIFI_SSID, tvSSID.getText().toString());
            toConfigNetPageIntent.putExtra(WIFI_PASSWORD, TextUtils.isEmpty(edtPassword.getText().toString()) ? "smile"
                    : edtPassword.getText().toString());
            toConfigNetPageIntent.putExtra(SeriesNumSearchActivity.BUNDE_SERIANO, seriaNo);
            toConfigNetPageIntent.putExtra(SeriesNumSearchActivity.BUNDE_VERYCODE, veryCode);
            toConfigNetPageIntent.putExtra(SUPPORT_NET_WORK, isSupportNetWork);
            toConfigNetPageIntent.putExtra(ResetIntroduceActivity.IS_FROM_DEVICE_SETTING, isFromDeviceSetting);
            toConfigNetPageIntent.putExtra(DEVICE_TYPE, deviceType);
            startActivity(toConfigNetPageIntent);
        }

        if (support_unknow_mode || notFindAvailableConfigNetWay){
            //若设备指示灯红蓝闪烁，请选择
